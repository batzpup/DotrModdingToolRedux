...
public class BatzpupMods
{
    ...
    private void ReadBatzpupModsFromIso()
    {
        ...
        cbExpandedZoom.Checked = new GameplayPatches.ExpandedZoom().IsApplied();
        cbRemoveRNGFromSlots.Checked = new GameplayPatches.RemoveRNGFromSlots().IsApplied();
        ...
        cbAllFusions.Checked = dataAccess.CheckIfPatchApplied(Patcher.AllowAllHandFusions.Offset, Patcher.AllowAllHandFusions.Patch) ||
        ...
        cbFixSlotsExtraSlotsAbility.Checked = new FixExtraCardLeaderAbility().IsApplied();
        cbVariousAIFixes.Checked = CheckForVariousAIFixes();
    ...
    private void ReadValuesFromIso()
    {
        ...
        cbTerrainChangeEnabled.Checked = !dataAccess.CheckIfPatchApplied(ChangeTerrainValues,
        ...
        cbAddAddtionalSlotRewards.Checked =
            dataAccess.CheckIfPatchApplied(AddSlotRewardsPtr, new byte[8] { 0x44, 0xc9, 0x05, 0x08, 0x00, 0x00, 0x00, 0x00 });
        ...
        cbUseCustomMusic.Checked =
            dataAccess.CheckIfPatchApplied(AddCustomMusicPtr, new byte[8] { 0xc0, 0xfd, 0x09, 0x08, 0x15, 0x00, 0x03, 0x24 });
    ...
    private void HandleMainSave()
    {
        ...
        new GameplayPatches.AIFasterTurnPassing().ApplyOrRemove(cbAIInputFix.Checked);
        new GameplayPatches.FastIntro().ApplyOrRemove(cbFastIntro.Checked);
        new GameplayPatches.AiGiveJoeyReviveMission().ApplyOrRemove(cbFastIntro.Checked);
        ...
        new GameplayPatches.FixExtraCardLeaderAbility().ApplyOrRemove(cbFixSlotsExtraSlotsAbility.Checked);
        new GameplayPatches.AiFixDarkhole().ApplyOrRemove(cbVariousAIFixes.Checked);
        new GameplayPatches.FixFeatherDusterUsage().ApplyOrRemove(cbVariousAIFixes.Checked);
        new GameplayPatches.AiFixGetMaxApAndCtlList().ApplyOrRemove(cbVariousAIFixes.Checked);
        new GameplayPatches.AiFixMakoHeavyStorm().ApplyOrRemove(cbVariousAIFixes.Checked);
        new GameplayPatches.AiPegasusNoFusions().ApplyOrRemove(cbVariousAIFixes.Checked);
        new GameplayPatches.AiDMKFixRevive().ApplyOrRemove(cbVariousAIFixes.Checked);

        new GameplayPatches.AiVanillaNoReviveEquips().ApplyOrRemove(cbFastIntro.Checked);
        ...
        else
        {
            dataAccess.ApplyPatch(MakeWhiteStart,
                new byte[36] {
                    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x28, 0x00, 0x00, 0x40, 0x79, 0x07, 0x0c, 0x01, 0x00, 0x04, 0x24, 0x3c,
                    0x01, 0x04, 0x8e, 0x40, 0x01, 0x05, 0x8e, 0x40, 0xb7, 0x06, 0x0c, 0x28, 0x36, 0x40, 0x72
                });
    ...
    {
        return cbAddAddtionalSlotRewards.Checked || cbAIInputFix.Checked || cbUseCustomMusic.Checked || cbAllowAllCustomDuels.Checked ||
               cbNoDCRequirementPostGame.Checked || cbFastIntro.Checked || cbForceNewGameSide.Checked;
    }

    private bool CheckForVariousAIFixes()
    {
        return new AiFixDarkhole().IsApplied() || new AiDMKFixRevive().IsApplied() || new FixFeatherDusterUsage().IsApplied() ||
               new AiFixGetMaxApAndCtlList().IsApplied();
    }
