...
public class Ai
{
    ...
    public string HexPointerAddress { get; }
    ...
    {
        this.Id = id;
        HexPointerAddress = hexPointerAddress;
        byte[] bytes = Enumerable.Range(0, HexPointerAddress.Length)
            .Where(x => x % 2 == 0)
            .Select(x => Convert.ToByte(HexPointerAddress.Substring(x, 2), 16))
            .ToArray();
        this.Bytes = bytes;
        this.Name = name;
    }
    ...
    public static Ai GetAiById(int id)
    {

        Ai matchedAi = Ai.All.First(x => x.Id == id);
        return matchedAi;
    }
