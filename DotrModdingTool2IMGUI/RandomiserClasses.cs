...
public class RandomiserChangeLog
{
    ...
    [JsonPropertyName("map_changes")] public RandomiserMapChanges RandomiserMapChanges { get; set; } = new();
    ...
    public string ToReadableFormat()
    {
        ...
        if (RandomiserMapChanges.MapSwaps?.Any() == true)
        {
            sb.AppendLine("Map Swaps:");
            sb.AppendLine();

            foreach (var swap in RandomiserMapChanges.MapSwaps)
            {
        ...
        if (RandomiserMapChanges.TerrainChanges?.Any() == true)
        {
            sb.AppendLine("Terrain Changes:");
            sb.AppendLine();
            foreach (var terrainChange in RandomiserMapChanges.TerrainChanges)
            {
        ...
        if (RandomiserMapChanges.HiddenCardChanges?.Any() == true)
        {
            sb.AppendLine("Hidden Card Changes:");
            sb.AppendLine();

            foreach (var hiddenChange in RandomiserMapChanges.HiddenCardChanges)
            {
    ...
    public string GetSummary(StringBuilder sb)
    {
        ...
        sb.AppendLine($"Map Swaps: {RandomiserMapChanges?.MapSwaps?.Count ?? 0}");
        sb.AppendLine($"Terrain Changes: {RandomiserMapChanges?.TerrainChanges?.Count ?? 0}");
        sb.AppendLine($"Hidden Card Changes: {RandomiserMapChanges?.HiddenCardChanges?.Count ?? 0}");
...
public class RandomiserMapChanges
{
